...
function gridz_footer_credit() {
    $output = '';
    $output .= __('Copyright Concept7 &copy;','gridz').' '.get_bloginfo('name');
    echo $output;
}
 ...
 */
function gridz_featured_image($size = 'full', $link = true, $class = 'entry-featured entry-featured-image', $postImage = null) {
    if(has_post_thumbnail()) {
      ...
      if(trim($image) != "") {
          ...
          else if($postImage) echo '<div style="height: 770px; background:url('.$feat_image.') no-repeat center center" class="'.$postImage.'"></div>';
...
function gridz_continue_reading_link() {
    return '<a class="more-link" href="'.get_permalink().'">'.__('...','gridz').'</a>';
}
